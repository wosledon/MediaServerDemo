...
namespace MediaServerDemo.Controllers
{
    ...
    {
        private static readonly ConcurrentDictionary<string, Channel<ReadOnlyMemory<byte>>> Channels = new ConcurrentDictionary<string, Channel<ReadOnlyMemory<byte>>>();
        ...
        {
            if (!Channels.TryGetValue(id, out var channel) || !channel.Writer.TryWrite(new ReadOnlyMemory<byte>([])))
            {
            ...
            try
            {
                ...
                {
                    var chunk = data.AsMemory(0, bytesRead).ToArray();
            ...
            if (Channels.TryGetValue(id, out var channel))
            {
                return new PushStreamResult(async outputStream =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            var c = buffer.Slice(0, buffer.Length);
                            await outputStream.WriteAsync(c);
